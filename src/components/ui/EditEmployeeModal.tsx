...
interface EditEmployeeModalProps {
  ...
  onSave: (
    num_empleado: string,
    fields: Partial<Pick<Employee, 'nombre' | 'area' | 'seccion' | 'puesto' | 'categoria' | 'turno' | 'fecha_ingreso' | 'ruta' | 'parada' | 'proyecto'>>
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const payload = {
        ...
        proyecto: form.proyecto ? form.proyecto : null,
      };
      const result = await onSave(employee.num_empleado, payload);
      ...
      <form id="edit-employee-form" onSubmit={handleSubmit} className="modal-body" noValidate>
        <div className="form-grid">
          ...
          {activeProjects.length > 0 && (
            <div className="form-group">
              <label htmlFor="edit-emp-proyecto">Asignar a Proyecto</label>
              <CustomSelect
                id="edit-emp-proyecto"
                value={form.proyecto}
                onChange={(val) => setForm({ ...form, proyecto: val })}
                options={[
                  { value: '', label: 'Ninguno (Vacante normal)' },
                  ...activeProjects.map((p) => ({ value: p, label: p }))
                ]}
                placeholder="Seleccione proyecto si aplica..."
              />
            </div>
          )}
        </div>
