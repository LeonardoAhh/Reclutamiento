...
function buildWhatsappMessageBlock(
  ...
  if (type === "general") {
    lines.push(
      `Total: ${totalVacantes} (${totalActivas} activas · ${totalBackup} backup)`,
    );
    lines.push(`Ingresos: ${totalProximos}`);
    lines.push(`Balance: Faltan ${vacantesNetas} por cubrir`);
  } else {
    lines.push(
      `Avance: ${totalStarliteEmpleados}/${totalStarliteUrgentes} contratados`,
    );
    const ingMsg =
      totalProximos > 1
        ? ` (${totalProximos} ingresos programados)`
        : totalProximos === 1
    ...
    for (const [puesto, filas] of puestosMap.entries()) {
      for (const r of filas) {
        ...
        if (
          r.turno &&
          cleanSeccion &&
          !normalizeForMatch(cleanSeccion).includes(normalizeForMatch(r.turno))
        ) {
          ...
          if (reqTotal > 0) {
            ...
            if (faltan > 0) {
              if (r.vacantesAutorizada === 0) {
                faltanTexto =
                  faltan === 1 ? `Falta 1 backup` : `Faltan ${faltan} backup`;
              } else if (r.vacantesBackup === 0) {
                faltanTexto =
                  faltan === 1 ? `Falta 1 activa` : `Faltan ${faltan} activas`;
              } else {
            ...
            faltanTexto =
              ingresosDisponibles === 1
                ? `1 ingreso extra`
                : `${ingresosDisponibles} ingresos extra`;
          }
        } else {
...
export function VacancyReportModal({
  ...
  const message = useMemo(
    () => buildWhatsappMessage(groups, dismissedKeys),
    [groups, dismissedKeys],
  );
    ...
    <ul className="vacancy-report-modal__rows">
      {group.rows.map((row) => (
        <li
          ...
          onClick={() =>
            toggleDismiss(`${row.area}|${row.seccion}|${row.puesto}`)
          }
        ...
        >
            ...
            <span className="vacancy-report-modal__seccion">
              ...
              {row.turno &&
                !row.seccion
                  .toUpperCase()
                  .replace(/[.\s]/g, "")
                  .includes(row.turno.toUpperCase().replace(/[.\s]/g, "")) && (
                  <> · {row.turno}</>
                )}
            </span>
          </div>
          <div className="vacancy-report-modal__badges" style={{ gap: "6px" }}>
                ...
                style={
                  row.vacantesStarlite === 0
                    ...
                    : {
                        ...
                        backgroundColor:
                          "color-mix(in srgb, var(--color-accent-orange) 12%, var(--color-canvas))",
                      }
                }
              >
            ...
            {row.vacantesAutorizada > 0 && (
              ...
              >
                {row.vacantesAutorizada} activa
                {row.vacantesAutorizada === 1 ? "" : "s"}
              </span>
            )}
      ...
      <div className="modal-body vacancy-report-modal__body">
        ...
        >
          <div className="vacancy-report-modal__stat">
            <div className="vacancy-report-modal__big-number">
              {totalActivas}
            </div>
            <p className="vacancy-report-modal__big-label">
              vacante{totalActivas === 1 ? "" : "s"} activa
              {totalActivas === 1 ? "" : "s"}
            </p>
          </div>
          <div className="vacancy-report-modal__stat">
          ...
          {totalStarliteUrgentes > 0 && (
            <div className="vacancy-report-modal__stat">
              <div
                className="vacancy-report-modal__big-number"
                style={{
                  color:
              ...
              <p className="vacancy-report-modal__big-label">Starlite</p>
            </div>
          )}
          {!isMobile && (
            <>
        ...
        {empty ? (
          ...
          <>
            ...
            {!isMobile && (
              ...
              >
                <h4 className="vacancy-report-modal__preview-title">
                  Mensaje para WhatsApp
                </h4>
                <pre className="vacancy-report-modal__preview-text">
                  {message}
                </pre>
              </motion.section>
            )}
      ...
      <footer className="modal-footer vacancy-report-modal__footer">
        ...
        >
          <span className="vacancy-report-modal__action-inner">
            ...
            {copied ? "Copiado" : isMobile ? "Copiar" : "Copiar"}
          </span>
        </button>
        <motion.button
          type="button"
