...
export function Dashboard() {
  ...
  const { coverVacancyForEmployee, vacancies, addVacancy, updateProjectStartDate } = useVacancyRequests();
  ...
  // Project Modal State
  const [startProjectTarget, setStartProjectTarget] = useState<string | null>(null);
  const [projectStartDate, setProjectStartDate] = useState(localTodayIso());
  ...
  // Agrupamos las vacantes de tipo 'proyecto' que estén activas
  const projectVacancies = useMemo(() => {
    const activeStatus = ['abierta', 'en_proceso', 'pausa'];
    ...
    for (const v of vacancies) {
      if (v.tipo === 'proyecto' && v.proyecto && activeStatus.includes(v.status)) {
        const p = projects.get(v.proyecto) ?? {
          nombre: v.proyecto,
          plazas: 0,
          inicio: v.fecha_inicio_proyecto ?? null,
          puestos: new Set<string>(),
          prioridad: v.prioridad,
        };
        p.plazas += 1;
        if (v.puesto) p.puestos.add(v.puesto);
        projects.set(v.proyecto, p);
      }
    }
      ...
      <section className="dashboard__hero" id="dashboard-hero">
        ...
        <div className="dashboard__hero-actions">
          <button
            ...
            title="Nueva vacante de proyecto"
          >
            <Rocket size={16} aria-hidden="true" />
            <span className="sr-only">Nueva vacante de proyecto</span>
          </button>
          <button
            type="button"
  ...
  <span style={{ display: 'none' }}>
    <JsonImporter onImport={handleImport} />
      ...
      <section className="dashboard__controls" id="dashboard-controls">
        ...
        <Modal
          isOpen={true}
          title={`Gestionar proyecto: ${startProjectTarget}`}
          onClose={() => setStartProjectTarget(null)}
          footerActions={
            <>
              <button onClick={() => setStartProjectTarget(null)} className="btn btn-secondary">
                Cancelar
              </button>
              <button
                className="btn btn-primary"
                onClick={async () => {
                  const result = await updateProjectStartDate(startProjectTarget, projectStartDate);
                  if (result.ok === false) {
        ...
        >
          <div className="modal-body">
            <div className="form-group">
              <label htmlFor="proj-start">Fecha de inicio oficial</label>
              <input
                id="proj-start"
                type="date"
              ...
              />
              <p className="form-hint" style={{ marginTop: '0.5rem', lineHeight: 1.4 }}>
                A partir de la fecha seleccionada comenzará a correr automáticamente el reloj de SLA (días hábiles) para todas las plazas asociadas a este proyecto.
              </p>
            </div>
          </div>
        </Modal>
      ...
      {/* ── Special Projects ── */}
      {projectVacancies.length > 0 && !filterArea && !searchTerm && (
        <motion.section 
        ...
        >
          <div className="dashboard__projects-header">
            <h2 id="projects-heading">
              <Rocket size={24} className="color-primary" aria-hidden="true" />
              <span>Proyectos especiales</span>
            </h2>
            <span className="dashboard__projects-count">{projectVacancies.length} activo{projectVacancies.length !== 1 ? 's' : ''}</span>
          </div>
          <motion.div 
            className="dashboard__projects-grid"
            initial="hidden"
            animate="show"
            variants={{
              hidden: { opacity: 0 },
          ...
          >
            {projectVacancies.map((p) => {
              const puestosList = Array.from(p.puestos).join(', ');
              const hasStarted = Boolean(p.inicio);
              return (
                <motion.div 
                  key={p.nombre} 
                  ...
                  whileHover={{ y: -4 }}
                  onClick={() => {
                    setStartProjectTarget(p.nombre);
                    setProjectStartDate(p.inicio || localTodayIso());
                  }}
                >
                  <div className="project-card__main">
                    <h3 className="project-card__title">{p.nombre}</h3>
                    <Badge variant="blue">{p.plazas} plaza{p.plazas !== 1 ? 's' : ''}</Badge>
                  </div>
                  <p className="project-card__puestos" title={puestosList}>{puestosList}</p>
                  <div className="project-card__meta">
                    <span className="project-card__sla">
                      {hasStarted ? `Inició: ${p.inicio}` : 'SLA: Pendiente inicio'}
                    </span>
                    <Badge variant={p.prioridad === 'alta' ? 'coral' : 'neutral'}>
      ...
      <VacancyModal
        ...
        onSave={async (payload) => {
          return await addVacancy(payload);
        }}
      />
      ...
      <EmployeeModal
        ...
        activeProjects={projectVacancies.map(p => p.nombre)}
