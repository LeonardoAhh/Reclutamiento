...
function ImageLightbox({ src, alt, onClose }: { src: string; alt: string; onClose: () => void }) {
  ...
  return (
    <div className="lightbox-overlay" onClick={onClose} role="dialog" aria-label="Visor de imagen">
...
export function Actividades() {
  ...
  const [rutinariasCollapsed, setRutinariasCollapsed] = useState(false);
  const [actividadesCollapsed, setActividadesCollapsed] = useState(false);
  ...
  const [editExistingReferenceImage, setEditExistingReferenceImage] = useState<string | null>(null);
  ...
  const handleEdit = async (e: React.FormEvent) => {
    ...
    if (editReferenceImageFile) {
      const url = await uploadReferenceImage(selectedActivity.id!, editReferenceImageFile);
      if (url) {
        newImageUrl = url;
      }
    }
  ...
  return (
    <div className="actividades-page">
      ...
      <div className="actividades-layout">
          ...
          >
            <div>
              <h2 className="actividades-section__title">
                {rutinariasCollapsed ? <ChevronRight size={18} aria-hidden="true" /> : <ChevronDown size={18} aria-hidden="true" />}
                Responsabilidades
                <span className="actividades-section__count">{rutinarias.length}</span>
              </h2>
              ...
              {rutinarias.length === 0 ? (
                <div className="actividades-empty">
                  <ListTodo
                    ...
                    aria-hidden="true"
                  />
                  <p className="actividades-empty__title">Sin responsabilidades</p>
                ...
                <>
                      ...
                      <div key={act.id} className="rutinaria-card">
                        <div className="rutinaria-card-main">
                          ...
                          <div className="rutinaria-content">
                            ...
                            {act.descripcion && (
                              <p className="rutinaria-desc">{act.descripcion}</p>
                            )}
                        ...
                        {act.reference_image && (
                          <div className="rutinaria-card-right">
                            <img src={act.reference_image} alt="Referencia visual" className="rutinaria-ref-img" onClick={(e) => { e.stopPropagation(); setLightboxSrc(act.reference_image!); }} />
                          </div>
                        )}
                        {isAdmin && (
                          <div
                  ...
                  {rutinarias.length > 4 && (
                    <button
                      ...
                      onClick={() => setRutinariasLimit(rutinariasLimit <= 4 ? rutinarias.length : 4)}
                    >
                      {rutinariasLimit <= 4 ? `Ver todas (${rutinarias.length})` : "Ver menos"}
                    </button>
                  )}
                </>
              )}
        ...
        <section className="actividades-section">
          ...
          >
            <div>
              <h2 className="actividades-section__title">
                {actividadesCollapsed ? <ChevronRight size={18} aria-hidden="true" /> : <ChevronDown size={18} aria-hidden="true" />}
                Actividades
                <span className="actividades-section__count">{allUnicas.length}</span>
              </h2>
              <p className="actividades-section__desc">
            ...
            <div className="actividades-grid">
              ...
              {unicas.length > 4 && (
                <button
                  type="button"
                  className="btn-ghost actividades-show-more"
                  onClick={() => setActividadesLimit(actividadesLimit <= 4 ? unicas.length : 4)}
                >
                  {actividadesLimit <= 4 ? `Ver todas (${unicas.length})` : "Ver menos"}
                </button>
              )}
            </>
          )}
            ...
            <div className="reference-upload-area">
              {referenceImagePreview ? (
                <div className="reference-preview">
                  ...
                  <button type="button" className="btn-icon-danger" onClick={() => { setReferenceImageFile(null); setReferenceImagePreview(null); }}>
          ...
          <div className="form-group">
            ...
            <div className="reference-upload-area">
              {editReferenceImagePreview || editExistingReferenceImage ? (
                <div className="reference-preview">
                  <img src={editReferenceImagePreview || editExistingReferenceImage!} alt="Preview" />
              ...
              <div className="form-group">
                ...
                {selectedActivity.reference_image && (
                  <div className="activity-reference-image">
                    ...
                    <img src={selectedActivity.reference_image} alt="Referencia visual" onClick={() => setLightboxSrc(selectedActivity.reference_image!)} />
                  </div>
                )}
              </div>
            </div>
      ...
      {lightboxSrc && (
        <ImageLightbox src={lightboxSrc} alt="Referencia visual" onClose={() => setLightboxSrc(null)} />
      )}
    </div>
  );
}
