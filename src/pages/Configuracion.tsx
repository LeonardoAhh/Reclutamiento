...
// Códigos que NO se consideran incidencias reales
const NON_INCIDENT_CODES = new Set(["-", "X", "A", "D", "DF", "B"]);

// Paleta decorativa "sticker" — solo para avatares, nunca para estructura/CTAs
...
function MiniCalendar({ days, mesStr }: { days: Record<string, string> | undefined, mesStr: string }) {
  ...
  const startOffset = firstDay === 0 ? 6 : firstDay - 1; // Lunes como inicio
  ...
  // Este calendario es un resumen visual redundante: la misma información
  // ya está disponible en texto en la lista de incidencias contigua.
  // Se oculta de lectores de pantalla para no duplicar ni obligar a navegar
  // 28-31 celdas sin acción; la fuente accesible es la lista.
  return (
...
export function Configuracion() {
  ...
  // Cargar resumenes (los meses disponibles)
  useEffect(() => {
  ...
  return (
    <main className="container config-page" aria-labelledby="config-title">
