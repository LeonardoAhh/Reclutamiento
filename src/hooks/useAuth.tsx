    ...
    async function fetchProfile() {
      try {
        const profileResult = await supabase
          .from('profiles')
          .select('id, username, display_name, role, created_at, last_login_at, avatar_url, hire_date')
          .eq('id', session!.user.id)
          .single();
        let data = profileResult.data as Profile | null;
        let error = profileResult.error;

        // Ventana de compatibilidad durante el despliegue: la app puede
        // convivir brevemente con una base que aún no aplicó la migración 052.
        if (
          error &&
          ((error as { code?: string }).code === '42703' ||
            error.message.toLowerCase().includes('hire_date'))
        ) {
          ...
          data = legacyResult.data
            ? ({ ...legacyResult.data, hire_date: null } as Profile)
            : null;
