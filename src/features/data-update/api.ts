...
export function dataUpdateError(error: unknown): string {
  ...
  if (message.includes("DATA_UPDATE_SHIFT_INVALID_INPUT")) return "El archivo de turnos contiene datos inválidos.";
  if (message.includes("DATA_UPDATE_SHIFT_DUPLICATE")) return "El archivo contiene números de empleado duplicados.";
  if (message.includes("DATA_UPDATE_SHIFT_NOT_FOUND")) return "Un empleado ya no está en esta campaña. Recarga y revisa el archivo.";
  ...
  if (message.includes("bulk_update_data_update_shifts")) {
    return "La actualización de turnos aún no está habilitada. Aplica la migración 051.";
  }
...
export async function bulkUpdateDataUpdateShifts(
  campaignId: string,
  changes: Array<{ employeeNumber: string; shift: string; expectedVersion: number }>,
): Promise<number> {
  const { data, error } = await supabase.rpc("bulk_update_data_update_shifts", {
    p_campaign_id: campaignId,
    p_updates: changes.map(({ employeeNumber, shift, expectedVersion }) => ({
      employeeNumber,
      shift,
      expectedVersion,
    })),
  ...
  if (typeof data !== "number" || !Number.isSafeInteger(data) || data < 0) {
    throw new Error("No se pudo confirmar cuántos turnos se actualizaron. Recarga la campaña.");
  }
  return data;
}
