...
export interface Employee {
   ...
   */
  parada?: string | null;
  /** Proyecto o centro de costos al que pertenece el empleado. */
...
/** Distinguishes an organic replacement vacancy from a project-driven expansion. */
export const VACANCY_TYPES = ['organica', 'proyecto'] as const;
export type VacancyType = (typeof VACANCY_TYPES)[number];
export const VACANCY_TYPE_LABEL: Record<VacancyType, string> = {
  organica: 'Orgánica',
...
export interface VacancyRequest {
  ...
  // ─── Campos de proyecto (aditivos, no rompen registros orgánicos) ────────
  /**
   * Origen de la vacante.
   * 'organica' (default) = reposición de baja o apertura de plantilla.
   * 'proyecto' = expansión por nuevo negocio, independiente de plantilla_autorizada.
   */
  tipo?: VacancyType | null;
  /**
   * Nombre del proyecto cuando tipo === 'proyecto'.
   * Ejemplo: 'StarLight', 'Ford Q3 2026'.
   */
  proyecto?: string | null;
  /**
   * Número de plazas abiertas en lote para este proyecto.
   * Default 1 (comportamiento orgánico habitual).
   * Al crear, se generan N registros individuales con este campo = 1
   * y el campo `proyecto` como agrupador.
   */
  cantidad?: number | null;
  /**
   * Fecha (YYYY-MM-DD) en que el proyecto arranca formalmente.
   * El SLA de estas vacantes se cuenta desde aquí, NO desde fecha_apertura.
   * Null = proyecto aún no iniciado; SLA aparece como "Pendiente".
   */
