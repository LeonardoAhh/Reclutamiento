...
## 2025-07 — Robustez de loaders con Morphicons

- Corregida la política global de `MorphingIcon`: ahora delega en Morphicons con `reducedMotion="user"`, evitando morphs cuando el sistema solicita movimiento reducido.
- Centralizados intervalo y duración total en `useMorphingSequence`; el hook normaliza valores inválidos, limpia el intervalo y expone siempre el primer icono bajo reduced motion.
- `SplashTypewriter` usa el ciclo central y elimina la espera animada bajo reduced motion; `TransitionLoader` conserva su estado accesible y fondo solido
- Movida la presentación de ambos loaders a CSS con tokens, safe areas, forced colors y media query de reduced motion; eliminados tamaños, colores, offsets, duraciones y easings inline.
- Verificación local: lint dirigido PASS, `yarn tsc -b --pretty false` PASS y `yarn vite build` PASS. Verificación de interfaz pendiente.
